...
option_list <- list(
  make_option("--sample-table", type = "character", default = "sample_table.csv",
              help = "Metadata table with columns: file_base, batch, is_anchor, sample_id, time_min, cytokine/stim"),
...
ff_example <- read.FCS(anchors_dt$file_full[1], transformation = FALSE, truncate_max_range = FALSE)
...
if (is.na(opt$channels)) {
  ...
  dropped <- setdiff(ch_vec, norm_channels)
  if (length(dropped)) {
    message("[WARN] These channels from ", opt$channels,
            " were not found or are technical and were dropped: ",
...
cat("\n=== Step 7: Dimensionality reduction & QC UMAP/t-SNE ===\n")
